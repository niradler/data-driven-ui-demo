...
export const resourceToType = (resource) => {
    return capitalize(pluralize.singular(resource));
}

export const getQueries = async (client) => {
    ...
    const queries = response.__type.fields.filter(({ name, args }) => !name.startsWith("node") && name !== 'query' && args.find(a => a.name == 'offset')).map(({ name }) => name);

    return queries;
};

export const getTypes = async (client) => {
    ...
    const objs = response.__schema.types.filter(({ kind, name }) => !name.startsWith("__") && kind === "OBJECT").map(({ name }) => name);
    debugger;
    return objs;
};

export const getFields = async (client, type) => {
    ...
    const fields = response.__type.fields.filter(({ name }) => !name.startsWith("node") && name != 'user' && name != 'comments').map(({ name }) => name);

    return fields;
};

const dataProvider = (client) => {
    return {
        getList: async ({ resource, pagination, sort, filters, metaData }) => {
            ...
            const fields = await getFields(client, resourceToType(resource))
